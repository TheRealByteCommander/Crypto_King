#!/bin/bash
# Fix Backend CORS und 500-Fehler (korrigierte Version)
...
# 1. Repository aktualisieren (lösche lokale Änderungen)
...
if ! grep -q "^CORS_ORIGINS" "${BACKEND_DIR}/.env" 2>/dev/null; then
    ...
    if ! grep -q "${CRYPTOKING_IP}:3000" "${BACKEND_DIR}/.env" 2>/dev/null; then
...
if grep -A 5 "app = FastAPI" "${BACKEND_DIR}/server.py" 2>/dev/null | grep -q "add_middleware.*CORSMiddleware"; then
...
if pgrep -f "uvicorn.*backend\|uvicorn.*server" > /dev/null 2>&1; then
...
if [ "$GET_RESPONSE" = "200" ]; then
    ...
    if [ -f "/var/log/supervisor/cyphertrade-backend.log" ]; then
        ...
        tail -20 /var/log/supervisor/cyphertrade-backend.log 2>/dev/null | grep -i "error\|exception\|traceback" | tail -5 || echo "  (keine Fehler gefunden)"
    fi
else
